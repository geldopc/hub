...
export function ProjectDetail() {
  ...
  const project = notionData?.find((n) => n.repo === repo);
  ...
  const chartData = stories && project
    ? buildExecutionChart(stories, taskDates, project.startDate, project.endDate)
    : [];
  ...
  const duration = formatDuration(project.startDate, project.endDate, project.status);
    ...
    <div id="project-detail" className="flex flex-col gap-8 py-8 px-4 md:px-8 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-start justify-between gap-4">
          <StatusBadge id="detail-status" status={project.status} />
          {storyCount > 0 && (
        ...
        <ProgressBar id="detail-progress" value={project.progress} />
